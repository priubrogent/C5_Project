...
WANDB = True  # Set to False to disable Weights & Biases logging
...
def train():
    ...
    config = run.config if WANDB else None
    ...
    lora_config = LoraConfig(
        r=config.lora_r if WANDB else LORA_R,
        lora_alpha=config.lora_alpha if WANDB else LORA_ALPHA,
        target_modules=[
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=config.batch_size if WANDB else BATCH_SIZE,
        per_device_eval_batch_size=config.batch_size if WANDB else BATCH_SIZE,
        num_train_epochs=config.epochs if WANDB else NUM_EPOCHS,
        learning_rate=config.learning_rate if WANDB else LEARNING_RATE,
        weight_decay=config.weight_decay if WANDB else WEIGHT_DECAY,
        warmup_ratio=config.warmup_ratio if WANDB else WARMUP_RATIO,
        lr_scheduler_type=config.lr_scheduler if WANDB else LR_SCHEDULER,
        optim=config.optimizer if WANDB else OPTIMIZER,
    ...
    if results_list:
        ...
        metrics=coco_evaluation(results_list, val_dataset.coco, OUTPUT_DIR, save=SAVE_FILES)
        fig=plot_loss(trainer, OUTPUT_DIR, save=SAVE_FILES)
        if WANDB:
            wandb.log(metrics)
